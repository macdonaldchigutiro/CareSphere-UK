...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      try {
        const token =
          localStorage.getItem("caresphere_access") ||
          sessionStorage.getItem("caresphere_access");
        ...
        if (!response.ok) {
          throw new Error(
            data?.detail || "Unable to load your CareSphere profile."
          );
        }
  ...
  const updateStoredUser = (userData) => {
    ...
    if (localStorage.getItem("caresphere_access")) {
      localStorage.setItem("caresphere_user", userString);
    }

    if (sessionStorage.getItem("caresphere_access")) {
      sessionStorage.setItem("caresphere_user", userString);
    }
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setProfile((current) => ({
        ...
        phone_number: data.phone_number || "",
        date_of_birth: data.date_of_birth || "",
        user_type: data.user_type || current.user_type,
        is_verified: Boolean(data.is_verified),
      }));
          ...
          <div className="flex flex-col justify-between gap-6 md:flex-row md:items-center">
            <div>
              ...
              <p className="mt-4 max-w-2xl text-lg leading-8 text-slate-300">
                Keep your personal information accurate so CareSphere can give
                you a better and more personalised experience.
              </p>
            </div>
          ...
          <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            <div className="mb-8">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-500">
                Update the details connected to your CareSphere account.
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <p className="mt-2 text-xs text-slate-400">
                  Email changes will be handled separately for account security.
                </p>
              </div>
          ...
          <aside className="space-y-6">
            ...
            <div className="rounded-[28px] bg-gradient-to-br from-[#0F766E] to-[#0A5B69] p-6 text-white shadow-xl">
              ...
              <p className="mt-3 text-sm leading-6 text-teal-50/90">
                CareSphere only uses your account information to support your
                care journey and personalise your experience.
              </p>
            </div>
          </aside>
        </div>
