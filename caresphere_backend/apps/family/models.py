...
class CareCircle(models.Model):
    ...
    consensus_threshold = models.IntegerField(
        default=75,
        help_text="Percentage of members required for consensus",
    )

    # Communication settings
    allow_external_invites = models.BooleanField(
        default=True,
    )

    auto_share_updates = models.BooleanField(
        default=True,
    )
    ...
    def __str__(self):
        return f"Care Circle for " f"{self.service_user.full_name or 'Service User'}"
...
class CareCircleMember(models.Model):
    ...
    user = models.ForeignKey(
        "users.User",
        on_delete=models.CASCADE,
        related_name="care_circle_memberships",
    )
    ...
    can_edit_profiles = models.BooleanField(
        default=False,
    )
    ...
    def __str__(self):
        return f"{self.user.get_full_name()} " f"in {self.care_circle}"
...
class FamilyDecision(models.Model):
    ...
    # Results
    chosen_option = models.JSONField(
        null=True,
        blank=True,
    )
...
class DecisionVote(models.Model):
    ...
    vote_weight = models.IntegerField(
        default=1,
    )
    ...
    def __str__(self):
        return (
            f"Vote by "
            f"{self.voter.user.get_full_name()} "
            f"on {self.decision.title}"
        )
...
class FamilyNote(models.Model):
    ...
    def __str__(self):
        return f"{self.title} by " f"{self.author.user.get_full_name()}"
    ...
    def can_view(
        ...
        if self.privacy_level == "restricted":
...
class CommunicationThread(models.Model):
    ...
    def __str__(self):
        return f"{self.subject} - " f"{self.care_circle}"
...
class ThreadMessage(models.Model):
    ...
    def __str__(self):
        return (
            f"Message by "
            f"{self.sender.user.get_full_name()} "
            f"in {self.thread.subject}"
        )
...
class MessageReadReceipt(models.Model):
    ...
    def __str__(self):
        return f"{self.reader.user.get_full_name()} " f"read at {self.read_at}"
...
class ThreadParticipation(models.Model):
    ...
    def __str__(self):
        return f"{self.participant.user.get_full_name()} " f"in {self.thread.subject}"
